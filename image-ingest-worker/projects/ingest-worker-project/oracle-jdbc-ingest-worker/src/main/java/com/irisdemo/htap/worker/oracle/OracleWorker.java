...
{
	protected static Logger logger = LoggerFactory.getLogger(OracleWorker.class);
    ...
    {	
		...
		workerDBUtils.setReadCommitted(connection);
		
		connection.setAutoCommit(false);
		
		PreparedStatement preparedStatement = connection.prepareStatement(config.getInsertStatement().replace("SpeedTest.", ""));
	...
	public void prepareDatabaseForSpeedTest() throws Exception
    {
		...
		try
		{
			if ( ! workerDBUtils.databaseExists(connection, "SPEED"))
			{
				workerDBUtils.createDatabase(connection, "SPEED", config.getDatabaseSizeInGB());
			}
