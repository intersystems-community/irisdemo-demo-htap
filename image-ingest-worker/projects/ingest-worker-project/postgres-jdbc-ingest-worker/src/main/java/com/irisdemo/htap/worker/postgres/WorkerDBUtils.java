...
public class WorkerDBUtils 
{
	...
	public void createDomain(Connection connection) throws SQLException, Exception
	{
		PreparedStatement statement = null;
		...
		{
			if (e.getMessage().contains("type \"datetime\" already exists"))
			{
	...
	public void alterRole(Connection connection) throws SQLException, Exception
	{
		PreparedStatement statement = null;

		logger.info("Altering Role...");
		String schemaName;
		String jdbcUsername =config.getIngestionJDBCUserName();
		...
		logger.info("Altering Role for schema " + schemaName);

		try
		{
			statement = connection.prepareStatement("ALTER ROLE "+ jdbcUsername + " SET search_path TO " + schemaName + ";");
		    statement.execute();
		}
		catch (SQLException e)
		{
	...
	public void createSchema(Connection connection) throws Exception, SQLException
	{
		...
		{
			if (e.getMessage().contains("schema \"" + schemaName.toLowerCase() + "\" already exists"))
			{
				// Good. It is there already.
			}
			else 
	...
	{		
		...
		try
		{
			...
			ResultSet rs = statement.executeQuery("SELECT setting from pg_settings where name = 'data_directory'");
			if (rs.next())
			{ 
	...
	public boolean databaseExists(Connection connection, String databaseName) throws SQLException
	{
		...
		try
		{
			...
			ResultSet rs = statement.executeQuery("SELECT datname FROM pg_catalog.pg_database where lower(datname) = lower('" + databaseName +"')");
