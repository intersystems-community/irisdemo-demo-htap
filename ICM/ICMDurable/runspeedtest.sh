...
# Worker 3
icm run -stateDir /ICMDurable/State \
    --machine asamary-CN-IRISSpeedTest-0005 \
    --container htapworker3 \
    --image intersystemsdc/irisdemo-demo-htap:iris-jdbc-ingest-worker-version-1.0 \
    --options "-p 80:8080 -e JAVA_OPTS=-Xmx${JAVA_XMX} -e MASTER_HOSTNAME=${MASTER_HOSTNAME} -e MASTER_PORT=${MASTER_PORT}"

# Worker 4
icm run -stateDir /ICMDurable/State \
    --machine asamary-CN-IRISSpeedTest-0006 \
    --container htapworker4 \
    --image intersystemsdc/irisdemo-demo-htap:iris-jdbc-ingest-worker-version-1.0 \
    --options "-p 80:8080 -e JAVA_OPTS=-Xmx${JAVA_XMX} -e MASTER_HOSTNAME=${MASTER_HOSTNAME} -e MASTER_PORT=${MASTER_PORT}"

# Worker 5
icm run -stateDir /ICMDurable/State \
    --machine asamary-CN-IRISSpeedTest-0007 \
    --container htapworker5 \
    --image intersystemsdc/irisdemo-demo-htap:iris-jdbc-ingest-worker-version-1.0 \
    --options "-p 80:8080 -e JAVA_OPTS=-Xmx${JAVA_XMX} -e MASTER_HOSTNAME=${MASTER_HOSTNAME} -e MASTER_PORT=${MASTER_PORT}"
